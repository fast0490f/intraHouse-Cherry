...
function getLinuxUrl {
  ...
  if [[ $check != "false" ]]; then
      url="https://raw.githubusercontent.com/fast0490f/intraHouse-Cherry/master/install_linux_deb.sh"
  else
      url="https://raw.githubusercontent.com/fast0490f/intraHouse-Cherry/master/install_linux_red.sh"
  fi
}

case "$OSTYPE" in
  solaris*) echo -e "\033[0;33m Error:\033[0;31m Installation is not supported\033[0;35m $OSTYPE!" && exit ;; #SOLARIS
  darwin*)  url="https://raw.githubusercontent.com/fast0490f/intraHouse-Cherry/master/install_darwin.sh" ;; #OSX
